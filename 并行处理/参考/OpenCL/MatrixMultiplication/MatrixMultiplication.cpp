...
initializeHost(void)
{
	...
	cl_uint Bsize = wB*hB;
	cl_uint Csize = wA*hB;
    inputA = (cl_float *) malloc(Asize*sizeof(cl_float));
    inputB = (cl_float *) malloc(Bsize*sizeof(cl_float));
    ...
    output = (cl_float *) malloc(Csize*sizeof(cl_float));
    if(output == NULL)
    {
...
initializeCL(void)
{
    ...
    const char * filename  = "MatrixMultiplication.cl";
...
runCLKernels(void)
{
    ...
    cl_uint maxDims;
    cl_event events[2];
	size_t globalThreads[2] = {arraySize, arraySize};
	size_t localThreads[2] = {5, 5};
...
main(int argc, char * argv[])
{
    ...
    // Print output array
    print1DArray(std::string("Output"), output, wA*hB);
